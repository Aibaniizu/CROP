...
$c_green  = imagecolorallocate($j, 0, 255, 0);
$c_red    = imagecolorallocate($j, 255, 0, 0);

fill_percent($j, $percent, $c_green, $c_red);
...
/* ------------------------------------------------------------------------ */
/*
*  Funktio täyttää puretusta kuvasta $percent prosenttia $c_green-värillä
*  ja loput punaisella
*/

function fill_percent($image, $percent, $c_green, $c_red) {
    ...
    /* Prosenttilukema pakotetaan tarvittaessa oikealle välille */
    ...
    if($percent > 0) {
        ...
        imagefilledrectangle($image, $sx, $sy,
            $sx + $width, $sy + $ey, $c_green);// $ey = koko korkeus täytetään
    } else {
        $width = 0;
    }
